<template>
  <div v-if="keycloakReady">
    <v-btn v-if="authenticated" data-test="btn-base-auth-logout" dark outlined @click="logout">
      <span>Logout</span>
    </v-btn>
    <v-btn v-else-if="hasLogin" data-test="btn-base-auth-login" dark outlined @click="login">
      <span>Login</span>
    </v-btn>
  </div>
</template>
