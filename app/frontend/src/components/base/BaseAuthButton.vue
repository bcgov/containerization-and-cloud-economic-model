<template>
  <div v-if="keycloakReady">
    <v-btn v-if="authenticated" data-test="base-auth-logout" dark outlined @click="logout">
      <span>Logout</span>
    </v-btn>
    <v-btn v-else-if="hasLogin" data-test="base-auth-login" dark outlined @click="login">
      <span>Login</span>
    </v-btn>
  </div>
</template>
