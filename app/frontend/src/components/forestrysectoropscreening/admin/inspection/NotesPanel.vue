...
export default {
  ...
  methods: {
    ...
    getNotes() {
      this.loading = true;
      commonFormService
        .getNotes(FormNames.FORESTRYSECTOROPSCREENING, this.submissionId)
        .then(response => {
    ...
    async addNote() {
      try {
        ...
        const response = await commonFormService.addNote(FormNames.FORESTRYSECTOROPSCREENING, this.submissionId, body);
        if (!response.data) {
