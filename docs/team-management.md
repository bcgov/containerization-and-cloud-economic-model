...
For this example, the slug will be: `myform`.
...
1. Go to Clients
1. Create Client
    1. Client ID = comfort-myform (all lowercase)
    1. click Save
...
1. Go to Clients, select comfort-myform
    1. Go to Roles tab
    1. Click Add Role
...
When a user signs into COMFORT, they use the `comfort-frontend` client.  This client always asks for the `comfort` scope, which now includes comfort-myform and its action roles.  If a user is assigned to one of comfort-myform's roles, then we will see comfort-myform in the resource_access.
...
1. Go to Clients, select comfort-frontend
    1. Go to Client Scopes tab
        ...
        1. Ensure that aud contains comfort-myform
        1. Ensure that resource_access contains comfort-myform, and its roles contain: admin, editor, reviewer, viewer
